...
const l1ChainId = 1
const MINIMUM_WITHDRAWAL_AMOUNT = utils.parseEther(process.env.MINIMUM_WITHDRAWAL_AMOUNT || '0.02')
...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
    ...
    //L2 withdraw expect amount in Alice L2 account
    const recipientBalance = await token.balanceOf(recipient)
    expect(recipientBalance).to.be.equal(aliceWithdrawAmount)

    //L2 withdraw expect 0 in the bridge
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    // [assignment] complete code here
    // Alice deposits 0.08 ETH in L1 -> Alice sends 0.05 ETH to Bob in L2 -> Bob withdraws all his funds in L2 -> Alice withdraws all her remaining funds in L1 -> assert all relevant balances are correct.
    ...
    // Alice deposits into tornado pool
    const aliceDepositAmount = utils.parseEther('0.8')
    const aliceDepositUtxo = new Utxo({ amount: aliceDepositAmount, keypair: aliceKeypair })
    const { args, extData } = await prepareTransaction({
      tornadoPool,
      outputs: [aliceDepositUtxo],
    ...
    //// Alice sends 0.05 ETH to Bob in L2 ////
    ...
    const bobSendAmount = utils.parseEther('0.05')
    ...
    const aliceWithdrawAmount = utils.parseEther('0.03')
